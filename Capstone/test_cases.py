...
class TestMaybankDemos1(unittest.TestCase):
    def setUp(self):
        self.driver = webdriver.Chrome()
        self.states = {
            "Johor": ".sc-fPXMVe:nth-child(4) > div",
    ...
    def test_demo_buttons1(self):
        for state, css_selector in self.states.items():
            try:
                ...
                # Click on Demo Maybank button
                demo_button = WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable((By.CSS_SELECTOR, '.sc-fPXMVe:nth-child(4)'))
                )
                demo_button.click()
                time.sleep(1)
                # Add assertions here to verify that you have navigated correctly or any other functionality
                expected_url = 'https://getq04.qbe.ee/maybank/b'
                self.assertEqual(self.driver.current_url, expected_url)
...
class TestMaybankDemos2(unittest.TestCase):
    def setUp(self):
        self.driver = webdriver.Chrome()
        self.states = {
            "Johor": ".sc-fPXMVe:nth-child(4) > div",
    ...
    def test_demo_buttons1(self):
        for state, css_selector in self.states.items():
            try:
                ...
                # Click on Demo Maybank button
                demo_button = WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable((By.CSS_SELECTOR, '.sc-fPXMVe:nth-child(5)'))
                )
                demo_button.click()
                time.sleep(1)
                # Add assertions here to verify that you have navigated correctly or any other functionality
